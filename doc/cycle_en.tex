...
\item[\texttt{cycle\_init(cycle)}] Sets \verb|init| and then calls
  \verb|cycle_reset()|. Returns \verb|EM_ERR| on a null pointer, otherwise
  \verb|EM_OK|.
\item[\texttt{cycle\_reset(cycle)}] Resets \verb|subSlot| and \verb|cycle| to
  $0$. Requires a previously initialised structure, otherwise \verb|EM_ERR|.
\item[\texttt{cycle\_set\_slot(cycle, slot)}] Positions the counter at the
  ...
  beforehand with \verb|cycle_check_slot()|.
\item[\texttt{cycle\_check\_slot(slot)}] Validates a slot: odd values in the
  range $0 < \mathtt{slot} \le \mathtt{SLOT\_CNT}$ are allowed. Returns the
  slot or $-1$.
\item[\texttt{cycle\_check(cycle, rxSlot, ss)}] Checks whether the received
  slot \verb|rxSlot| matches the currently expected sub-slot
  (\verb|cycle->sSlot == rxSlot|). Invalid slots return \verb|false|.
\item[\texttt{cycle\_string(cycle)}] Returns a compact representation in the
  format \verb|(c:<cycle>, <slot>, <subSlot>)| in a static buffer.
