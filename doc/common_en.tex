...
\item[\texttt{idx2str(map, cnt, idx)}] Looks up \verb|idx| in a table of
  \verb|cnt| entries. Returns: \verb|char *| -- the associated string, or
  \verb|"NA "| if not found.
\item[\texttt{idxa2str(map, idx)}] Convenience for \verb|idxa2str_t|: calls
  \verb|idx2str()| with the data stored in \verb|map|. Returns: \verb|char *|.
\end{description}
\par}
...
\item[\texttt{clable2type(lbl)}] Determines whether the label is a hex number
  ...
  (\verb|nonasci|). Returns: \verb|type_e|.
\item[\texttt{clabel2uint(lbl)} / \texttt{str2uint(str)}] Interpret the label
  or a string as an unsigned integer. Returns: \verb|int8_t|.
\end{description}
\par}
...
\item[\texttt{to\_hex(out, out\_size, buffer, buffer\_size, write\_asci)}]
  ...
  optionally the ASCII representation. Returns: \verb|uint16_t| (number of
  characters written).
...
\item[\texttt{int2hchar(nr)}] Converts a value into the corresponding hex digit
  character (\texttt{0}--\texttt{9}, \texttt{A}--\texttt{F}). Returns:
  \verb|char|.
\end{description}
\par}
...
\item[\texttt{common\_crc16(data\_p, length)}] Computes a CRC-16 according to
  ...
  definition from Python's \verb|crcmod|. Returns: \verb|uint16_t| (checksum).
\item[\texttt{csss2uint32(cycle, slot, sSlot)}] Packs cycle ($16$ bit), slot
  ...
  Returns: \verb|uint32_t|.
\item[\texttt{swap(val)}] Swaps the byte order of a $32$-bit value (endianness
  reversal). Returns: \verb|uint32_t|.
\item[\texttt{modulo\_sub(slot, oSlot, modulo)}] Returns the modular
  difference of two slots. Marked as not yet tested in the source. Returns:
  \verb|uint8_t|.
\item[\texttt{ReadModify\_write(mem, add)}] Atomic read-modify-write:
  atomically adds \verb|add| to the byte at \verb|mem| via the exclusive
  load/store instructions (\verb|LDREX|/\verb|STREX|) with a memory barrier,
  retrying until the store succeeds. Returns: \verb|bool| (\verb|true| on
  success).
\end{description}
\par}
...
\item[\texttt{board\_get\_unique\_id(id, max\_len)}] Reads the 96-bit
  ...
  implementation in the \verb|UNIT_TEST| build. Returns: \verb|size_t| (number
  of values written).
\item[\texttt{in\_interrupt()}] Placeholder; currently always returns
  \verb|false|. Returns: \verb|int|.
\end{description}
\par}
