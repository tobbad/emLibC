...
\item[\texttt{buffer\_new(size, type)}] Alloziert eine \verb|buffer_t|\-Struktur
  samt Speicher der Größe \verb|size| und liefert einen Zeiger darauf bzw.
  \verb|NULL| bei \verb|size == 0| oder fehlgeschlagener Allokation.
\item[\texttt{buffer\_new\_buffer\_t(buffer)}] Re\-initialisiert eine bereits
  ...
  angelegt.
\item[\texttt{buffer\_free(buffer)}] Gibt Speicher und Struktur frei. Meldet
  \verb|EM_ERR|, wenn der Puffer noch als \verb|BUFFER_USED| markiert ist.
\end{description}
\par}
...
\item[\texttt{buffer\_set(buffer, data, size)}] Schreibt \verb|size| Bytes in
  ...
  setzt \verb|state = BUFFER_USED| und aktualisiert das Label.
\item[\texttt{buffer\_get(buffer, data, size)}] Liest bis zu \verb|*size| Bytes
  ...
  danach leer, wird \verb|state = BUFFER_READY|.
\item[\texttt{buffer\_transfer(from, to)}] Überträgt den Inhalt von \verb|from|
  ...
  Inhalt nicht, bleibt \verb|from| unangetastet und es wird \verb|EM_ERR|
  geliefert; andernfalls wird \verb|from| geleert.
\end{description}
\par}
...
\item[\texttt{buffer\_clear(buffer)}] Markiert den Inhalt als verbraucht
  ...
  den Speicher zu nullen.
\item[\texttt{buffer\_tolower(buffer)}] Wandelt im belegten Bereich alle
  Großbuchstaben \texttt{A}--\texttt{Z} in Kleinbuchstaben um.
