...
\item[\texttt{cycle\_init(cycle)}] Setzt \verb|init| und ruft anschliessend
  \verb|cycle_reset()| auf. Liefert \verb|EM_ERR| bei einem Nullzeiger, sonst
  \verb|EM_OK|.
\item[\texttt{cycle\_reset(cycle)}] Setzt \verb|subSlot| und \verb|cycle|
  auf $0$ zurück. Erfordert eine zuvor initialisierte Struktur, andernfalls
  \verb|EM_ERR|.
\item[\texttt{cycle\_set\_slot(cycle, slot)}] Positioniert den Zähler auf den
  ...
  zuvor mit \verb|cycle_check_slot()| geprüft.
\item[\texttt{cycle\_check\_slot(slot)}] Validiert einen Slot: zulässig sind
  ...
  Liefert den Slot zurück oder $-1$.
\item[\texttt{cycle\_check(cycle, rxSlot, ss)}] Prüft, ob der empfangene Slot
  \verb|rxSlot| dem aktuell erwarteten Sub\-Slot entspricht
  (\verb|cycle->sSlot == rxSlot|). Ungültige Slots liefern \verb|false|.
...
\item[\texttt{cycle\_print(cycle, title)}] Gibt alle Felder zeilenweise aus
  (Debug\-Hilfe, vgl. \verb|xx_print| Konvention in der Einführung).
\end{description}
