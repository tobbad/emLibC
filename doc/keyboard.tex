...
\item[\texttt{keyboard\_init(kybd, device)}] Registriert die Vtable
  \verb|kybd| und ruft deren \verb|init| auf. Rückgabe: \verb|dev_handle_t|
  (zugeteiltes Handle, $0$ bei Fehler bzw.\ voller Tabelle).
\item[\texttt{keyboard\_scan(dev)}] Pollt das Gerät und liefert die erkannte
  Taste. Rückgabe: \verb|int16_t| -- per Konvention bezeichnet ein negativer
  Wert eine eingegebene Zahl ($0\dots127$), ein positiver einen Buchstaben.
...
\item[\texttt{keyboard\_add(dev, add)}] Mischt einen Zustand additiv in den
  Gerätezustand. Rückgabe: \verb|void|.
\item[\texttt{keyboard\_diff(dev, ref, diff)}] Bestimmt die Differenz des
  Gerätezustands gegenüber \verb|ref|. Rückgabe: \verb|em_msg| (Ergebnis der
  Geräte\-Funktion; \verb|EM_ERR| bei ungültigem Handle).
\item[\texttt{keyboard\_isdirty(dev)} / \texttt{keyboard\_undirty(dev)}] Fragt
  das Änderungs\-Flag ab (\verb|bool|) bzw.\ setzt es zurück (\verb|void|).
