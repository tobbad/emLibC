...
\item[\texttt{idx2str(map, cnt, idx)}] Sucht \verb|idx| in einer Tabelle aus
  \verb|cnt| Einträgen. Rückgabe: \verb|char *| -- die zugehörige Zeichenkette
  bzw.\ \verb|"NA "|, falls nicht gefunden.
\item[\texttt{idxa2str(map, idx)}] Bequemlichkeit für \verb|idxa2str_t|: ruft
  \verb|idx2str()| mit den in \verb|map| hinterlegten Daten auf. Rückgabe:
  \verb|char *|.
\end{description}
\par}
...
\item[\texttt{clable2type(lbl)}] Bestimmt, ob das Label eine Hexzahl
  ...
  (\verb|nonasci|) ist. Rückgabe: \verb|type_e|.
\item[\texttt{clabel2uint(lbl)} / \texttt{str2uint(str)}] Interpretieren das
  Label bzw.\ eine Zeichenkette als vorzeichenlose Ganzzahl. Rückgabe:
  \verb|int8_t|.
\end{description}
\par}
...
\item[\texttt{to\_hex(out, out\_size, buffer, buffer\_size, write\_asci)}]
  ...
  die ASCII\-Darstellung. Rückgabe: \verb|uint16_t| (Anzahl geschriebener
  Zeichen).
...
\item[\texttt{common\_crc16(data\_p, length)}] Berechnet eine CRC\-16 nach
  ...
  Pythons \verb|crcmod|. Rückgabe: \verb|uint16_t| (Prüfsumme).
\item[\texttt{csss2uint32(cycle, slot, sSlot)}] Packt Cycle ($16$ Bit), Slot
  ...
  Rückgabe: \verb|uint32_t|.
\item[\texttt{swap(val)}] Vertauscht die Byte\-Reihenfolge eines
  $32$\-Bit\-Werts (Umkehr der Endianness). Rückgabe: \verb|uint32_t|.
\item[\texttt{modulo\_sub(slot, oSlot, modulo)}] Liefert die modulare Differenz
  zweier Slots. Im Quelltext als noch nicht getestet markiert. Rückgabe:
  \verb|uint8_t|.
\item[\texttt{ReadModify\_write(mem, add)}] Atomares Read\-Modify\-Write:
  addiert \verb|add| über die exklusiven Lade\-/Speicherbefehle
  (\verb|LDREX|/\verb|STREX|) mit Speicherbarriere atomar auf das Byte unter
  \verb|mem| und wiederholt, bis der Speichervorgang gelingt. Rückgabe:
  \verb|bool| (\verb|true| bei Erfolg).
\end{description}
\par}
...
\item[\texttt{board\_get\_unique\_id(id, max\_len)}] Liest die 96\-Bit
  ...
  eine deterministische Ersatzimplementierung ersetzt. Rückgabe: \verb|size_t|
  (Anzahl geschriebener Werte).
\item[\texttt{in\_interrupt()}] Platzhalter; liefert derzeit stets
  \verb|false|. Rückgabe: \verb|int|.
\end{description}
\par}
