...
{\sloppy\emergencystretch=3em
\begin{description}
\item[\texttt{serial\_mode\_set(mode)} / \texttt{serial\_mode\_get()}] Sets or
  reads the \verb|print_e| bit field.
\item[\texttt{serial\_reset(dev)}] Resets the RX/TX buffers and the device
  state.
\item[\texttt{serial\_waitForNumber(key)}] Returns the last received input as a
  number or $-1$; \verb|key| then points to the string.
\item[\texttt{serial\_get\_byte\_per\_second()} /
  \texttt{serial\_reset\_byte\_per\_second()}] Read or reset the measured
  transmit throughput.
\item[\texttt{serial\_uart\_printf(line, len)}] Prints \verb|len| bytes
  directly over the UART.
\end{description}
\par}
