...
\item[\texttt{keyboard\_init(kybd, device)}] Registers the vtable \verb|kybd|
  and calls its \verb|init|. Returns: \verb|dev_handle_t| (the assigned handle,
  $0$ on error or a full table).
\item[\texttt{keyboard\_scan(dev)}] Polls the device and returns the detected
  key. Returns: \verb|int16_t| -- by convention a negative value denotes an
  entered number ($0\dots127$), a positive one a letter.
\item[\texttt{keyboard\_state(dev, ret)} / \texttt{keyboard\_set\_state(dev, state)}]
  Reads the current key state into \verb|ret| or overwrites it. Returns:
  \verb|void|.
\item[\texttt{keyboard\_add(dev, add)}] Merges a state additively into the
  device state. Returns: \verb|void|.
\item[\texttt{keyboard\_diff(dev, ref, diff)}] Determines the difference of the
  device state relative to \verb|ref|. Returns: \verb|em_msg| (result of the
  device function; \verb|EM_ERR| on an invalid handle).
\item[\texttt{keyboard\_isdirty(dev)} / \texttt{keyboard\_undirty(dev)}]
  Queries the change flag (\verb|bool|) or resets it (\verb|void|).
