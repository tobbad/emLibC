...
\item[\texttt{time\_init()}] Initialisiert das Modul: aktiviert den
  ...
  Muss vor allen anderen Funktionen aufgerufen werden. Rückgabe: \verb|void|.
\item[\texttt{time\_new(name)}] Reserviert einen freien Handle und hinterlegt
  den Namen. Rückgabe: \verb|time_handle_t| (das Handle, bzw.\ $-1$, wenn
...
\item[\texttt{time\_set\_mode(hdl, mode)} / \texttt{time\_get\_mode(hdl)}]
  ...
  Rückgabe: \verb|em_msg| (Setzer; \verb|EM_ERR| bei ungültigem Handle) bzw.\
  \verb|mode_e| (die aktuelle Betriebsart).
\item[\texttt{time\_set\_max(hdl, max)} / \texttt{time\_get\_max(hdl)}]
  ...
  überschrieben. Rückgabe: \verb|em_msg| (Setzer) bzw.\ \verb|int8_t| (die
  gesetzte Grenze; \verb|EM_ERR| bei ungültigem Handle).
\item[\texttt{time\_reset(hdl)}] Löscht die Messungen des Handles, behält
  aber den Initialisierungszeitpunkt \verb|init_ns| bei. Rückgabe: \verb|void|.
\item[\texttt{time\_start(hdl, count, ptr, cycle)}] Startet eine Messung:
  ...
  \verb|SSSC| die Cycle\-Position aus \verb|cycle|. Rückgabe: \verb|void|.
\item[\texttt{time\_stop\_su(hdl)}] Erfasst den Zwischenstopp und berechnet
  \verb|duration_su_ns| (Zeit von Start bis \emph{setup}). Rückgabe:
  \verb|void|.
...
\item[\texttt{time\_auto(hdl, count, ptr, cycle)}] Bequemlichkeit:
  \verb|time_start()| unmittelbar gefolgt von \verb|time_stop()|. Rückgabe:
  \verb|void|.
\item[\texttt{time\_now\_ns()}] Liefert den aktuellen Zählerstand in
  Nanosekunden (\verb|NOW|). Rückgabe: \verb|int64_t|.
\item[\texttt{time\_doLoop\_get()}] Im \verb|ONESHOT|\-Betrieb wird
  \verb|doLoop| nach einem vollen Durchlauf \verb|false|. Rückgabe:
  \verb|bool|.
\item[\texttt{time\_print(hdl, titel, python, timing)}] Gibt den Pool als
  Tabelle aus. Siehe nächster Abschnitt. Rückgabe: \verb|void|.
\end{description}
