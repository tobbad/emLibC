...
static mkey_t reset_key = {
    .last = 0, .current = 0, .unstable = 0, .cnt = 0, .stable = true};
...
static xpad_dev_t default_xscan_dev = {
    ...
    .state =
        {
            .label = {'1', '2', '3', 'A', '4', '5', '6', 'B', '7', '8', '9',
                      'C', '0', 'F', 'E', 'D'},
            .state = {OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF,
                      OFF, OFF, OFF, OFF, OFF},
            ...
            .pin =
                {
                    {.port = GPIOC,
                     ...
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 1
                    {.port = GPIOC,
                     .pin = GPIO_PIN_2,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 2
                    {.port = GPIOC,
                     .pin = GPIO_PIN_3,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 3
                    {.port = GPIOA,
                     .pin = GPIO_PIN_0,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 4
                    {.port = GPIOA,
                     .pin = GPIO_PIN_4,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 5
                    {.port = GPIOA,
                     .pin = GPIO_PIN_5,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 6
                    {.port = GPIOC,
                     .pin = GPIO_PIN_15,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 7
                    {.port = GPIOC,
                     .pin = GPIO_PIN_9,
                     .conf = {.Mode = GPIO_MODE_INPUT,
                              .Pull = GPIO_PULLUP}}, // Taste 8
                },
        },
...
static uint16_t xpad_update_key(uint8_t devh, uint8_t zeile, bool pinVal) {
  ...
  if (my_xpad[devh].key[zeile].cnt > STABLE_CNT) {
    ...
    if (my_xpad[devh].key[zeile].stable) {
      my_xpad[devh].state.state[zeile] =
          ((my_xpad[devh].state.state[zeile] + 1) % STATE_CNT);
      // my_xpad[devh].state.dirty = true;
      my_xpad[devh].state.dirty = true;
      printf("Pushed   Key @ (zeile =%d, z=%d, s=%d, value = %c)" NL, zeile, z,
             s, label);
    } else {
...
static uint16_t xpad_spalten_scan(dev_handle_t devh) {
  ...
  if (my_xpad[devh].dev_type != XSCAN) {
    printf("Handle (%d) for this keyboard not valid" NL,
           my_xpad[devh].dev_type);
    return false;
  }
  int16_t res = 0;
