...
void time_start(time_handle_t hdl, uint8_t count, uint8_t *ptr) {
    ...
    if (_time.time[hdl].idx >= 0) {
        ...
        _time.time[hdl].measurement[_time.time[hdl].idx].start      = DWT->CYCCNT;
    }
}
void time_end_su(time_handle_t hdl) {
    if (time_check_hdl(hdl)==EM_ERR) return;
    if (_time.time[hdl].idx >= 0) {
        _time.time[hdl].measurement[_time.time[hdl].idx].stop_su        = DWT->CYCCNT;
        _time.time[hdl].measurement[_time.time[hdl].idx].duration_su_us = 125 * (_time.time[hdl].measurement[_time.time[hdl].idx].stop_su - _time.time[hdl].measurement[_time.time[hdl].idx].start) / 10000;
    }
}
...
void time_end_tx(time_handle_t hdl) {
    if (time_check_hdl(hdl)==EM_ERR) return;
    if (_time.time[hdl].idx >= 0) {
        _time.time[hdl].measurement[_time.time[hdl].idx].stop_tx        = DWT->CYCCNT;
        _time.time[hdl].measurement[_time.time[hdl].idx].duration_tx_us = 125 *( _time.time[hdl].measurement[_time.time[hdl].idx].stop_tx - _time.time[hdl].measurement[_time.time[hdl].idx].start) / 10000;
        _time.time[hdl].measurement[_time.time[hdl].idx].baud           = 1000000 * 10 * _time.time[hdl].measurement[_time.time[hdl].idx].count / _time.time[hdl].measurement[_time.time[hdl].idx].duration_tx_us;
        _time.time[hdl].measurement[_time.time[hdl].idx].tick_stop      = HAL_GetTick();
        _time.time[hdl].measurement[_time.time[hdl].idx].tick_duration  = _time.time[hdl].measurement[_time.time[hdl].idx].tick_stop - _time.time[hdl].measurement[_time.time[hdl].idx].tick_start;
        _time.time[hdl].idx = (_time.time[hdl].idx + 1) % MEAS_CNT;
        if ((_time.time[hdl].idx == 0) && (_time.time[hdl].idx & ONE_SHOT)) {
