...
void time_end_tx() {
    if (_time.idx >= 0) {
        ...
        _time.time[_time.idx].tick_duration = _time.time[_time.idx].tick_stop - _time.time[_time.idx].tick_start;
...
void time_print(char *titel) {
    ...
    for (uint8_t i = 0; i < MEAS_CNT; i++) {
        printf("[ %llu, %u, %ld ]," NL, _time.time[_time.idx].duration_tx_us, _time.time[_time.idx].count,
               _time.time[_time.idx].baud);
    }
    printf("]" NL);
}
