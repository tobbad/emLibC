...
em_msg rbuf_pull_line(rbuf_hdl_t hdl, uint8_t *bytes, int16_t *count) {
  ...
  if ((line_buffer.empty) ||
      (line_buffer.nxtLineWrIdx != line_buffer.nxtLineRdIdx)) {
    uint8_t toRead = MIN(*count, rbuf->buffer_size);
    *count = toRead;
    line_buffer.nxtLineWrIdx =
        (line_buffer.nxtLineWrIdx + 1) % line_buffer.valid_cnt;
...
em_msg rbuf_push_line(rbuf_hdl_t hdl, const uint8_t *bytes, int16_t count) {
  ...
  if (!line_buffer.empty) {
    ...
    line_buffer.nxtLineRdIdx =
        (line_buffer.nxtLineRdIdx + 1) % line_buffer.valid_cnt;
