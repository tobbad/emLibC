...
typedef enum { SLIP_ENCODE = 0, SLIP_DECODE = 1 } slip_codec_function_t;
...
em_msg slip_write(slip_handle_e hdl, const uint8_t *buffer, uint16_t length) {
  ...
  if ((hdl >= 0) && (hdl < SLIP_HANDLE_CNT)) {
    uint8_t value;
    uint8_t map_size = codec[hdl].set == SLIP_ESC_SIMPLE_SET
                           ? SLIP_SIMPLE_MAP_SIZE
                           : SLIP_MAP_SIZE;
        ...
        if (SLIP_STATE_DECODE_WFF == codec[hdl].state) {
          ...
          if (SLIP_PKT_LIMIT == value) {
            if (0 == codec[hdl].size) {
              ...
              DEB_DECODE(
                  "Ignore 2nd Start of frame detected (Zero packet size)\n");
              continue;
            } else {
